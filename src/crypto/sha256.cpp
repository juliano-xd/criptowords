...
static inline void sha256_init_simd(uint32_t (&state)[8][Lanes]) noexcept {
    for (unsigned i = 0; i < 8; ++i)
        for (unsigned lane = 0; lane < Lanes; ++lane)
            state[i][lane] = SHA256_IV[i];
}
...
// =========================================================================
[[gnu::target("sse4.1")]]
void sha256_transform_sse(SHA256_SSE_State* __restrict ctx, const uint32_t W_in[16][4]) {
    ...
    _mm_storeu_si128((__m128i*)ctx->state[0], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[0]), a));
    _mm_storeu_si128((__m128i*)ctx->state[1], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[1]), b));
    _mm_storeu_si128((__m128i*)ctx->state[2], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[2]), c));
    _mm_storeu_si128((__m128i*)ctx->state[3], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[3]), d));
    _mm_storeu_si128((__m128i*)ctx->state[4], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[4]), e));
    _mm_storeu_si128((__m128i*)ctx->state[5], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[5]), f));
    _mm_storeu_si128((__m128i*)ctx->state[6], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[6]), g));
    _mm_storeu_si128((__m128i*)ctx->state[7], _mm_add_epi32(_mm_loadu_si128((__m128i*)ctx->state[7]), h));
}
...
void sha256_transform_avx2(SHA256_AVX2_State* __restrict ctx, const uint32_t W_in[16][8]) {
    ...
    _mm256_storeu_si256((__m256i*)ctx->state[0], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[0]), a));
    _mm256_storeu_si256((__m256i*)ctx->state[1], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[1]), b));
    _mm256_storeu_si256((__m256i*)ctx->state[2], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[2]), c));
    _mm256_storeu_si256((__m256i*)ctx->state[3], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[3]), d));
    _mm256_storeu_si256((__m256i*)ctx->state[4], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[4]), e));
    _mm256_storeu_si256((__m256i*)ctx->state[5], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[5]), f));
    _mm256_storeu_si256((__m256i*)ctx->state[6], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[6]), g));
    _mm256_storeu_si256((__m256i*)ctx->state[7], _mm256_add_epi32(_mm256_loadu_si256((__m256i*)ctx->state[7]), h));
}
...
void sha256_transform_avx512(SHA256_AVX512_State* __restrict ctx, const uint32_t W_in[16][16]) {
    ...
    _mm512_storeu_si512((__m512i*)ctx->state[0], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[0]), a));
    _mm512_storeu_si512((__m512i*)ctx->state[1], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[1]), b));
    _mm512_storeu_si512((__m512i*)ctx->state[2], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[2]), c));
    _mm512_storeu_si512((__m512i*)ctx->state[3], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[3]), d));
    _mm512_storeu_si512((__m512i*)ctx->state[4], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[4]), e));
    _mm512_storeu_si512((__m512i*)ctx->state[5], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[5]), f));
    _mm512_storeu_si512((__m512i*)ctx->state[6], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[6]), g));
    _mm512_storeu_si512((__m512i*)ctx->state[7], _mm512_add_epi32(_mm512_loadu_si512((__m512i*)ctx->state[7]), h));
}
