...
void monitor_progress(const SearchState& state, size_t num_threads,
    ...
    while (!state.found.load(std::memory_order_relaxed) &&
        ...
        if (state.found.load(std::memory_order_relaxed) ||
            state.all_done.load(std::memory_order_relaxed)) break;
...
void BruteForceEngine::run(ExecutionPipeline& pipeline, size_t num_threads) {
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        workers.emplace_back(&BruteForceEngine::worker,
                             std::ref(pipeline), i, num_threads, std::ref(state));
    }
