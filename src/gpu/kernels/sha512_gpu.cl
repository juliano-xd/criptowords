...
__kernel void test_sha512(__global const uchar* input, const uint input_len, __global uchar* output) {
    ulong W[80];
    ulong state[8] = {
    ...
    // Pad block (128 bytes max)
    uchar padded[128];
    for(int i=0; i<128; i++) padded[i] = 0;
    
    // Copy input
    for(uint i=0; i<input_len && i<111; i++) padded[i] = input[i];
    
    // Add 0x80 bit
    padded[input_len] = 0x80;
    
    // Add bit length in big-endian at the end of block (bytes 124-127)
    uint bit_len = input_len * 8;
    padded[124] = (bit_len >> 24) & 0xFF;
    padded[125] = (bit_len >> 16) & 0xFF;
    padded[126] = (bit_len >> 8) & 0xFF;
    padded[127] = bit_len & 0xFF;

    // Convert to W array
    for(int i=0; i<16; i++) {
        W[i] =  ((ulong)padded[i*8 + 0] << 56) | ((ulong)padded[i*8 + 1] << 48) |
                ((ulong)padded[i*8 + 2] << 40) | ((ulong)padded[i*8 + 3] << 32) |
                ((ulong)padded[i*8 + 4] << 24) | ((ulong)padded[i*8 + 5] << 16) |
                ((ulong)padded[i*8 + 6] << 8)  | ((ulong)padded[i*8 + 7]);
    }
