...
FORCE_INLINE constexpr UInt<4> load_be256(const std::array<uint8_t, 32> &be) noexcept {
    return UInt<4>(be.data(), 32, BitOrder::MSB);
}

FORCE_INLINE constexpr UInt<4> load_be256(const uint8_t be[32]) noexcept {
    return UInt<4>(be, 32, BitOrder::MSB);
}
...
FORCE_INLINE constexpr bool secp256k1_tweak_add_fast(std::array<uint8_t, 32> &seckey, const std::array<uint8_t, 32> &tweak) noexcept {
    ...
    if (carry) {
        // Soma verdadeira é 2^256 + k >= 2^256 > n.
        // (2^256 + k) - n = k + (2^256 - n) = k + DELTA.
        k += DELTA;
    } else {
        if (k >= N_VAL) {
            k -= N_VAL;
    ...
    // BIP-32: chave privada resultante não pode ser 0
